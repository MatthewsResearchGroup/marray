...
class marray
: public marray_base<Type, NDim, marray<Type, NDim, Allocator>, true>
{
  ...
  public:
    ...
    marray(const std::vector<T>& v,
           std::enable_if_t<(NDim == 1 || NDim == DYNAMIC)
                            && std::is_convertible_v<T, value_type>>* = nullptr)
#else
    marray(const std::vector<T>& v)
#endif
    {
     ...
     * Copy constructor.
     *
     * If `other` is a tensor ([marray](@ref MArray::marray)), then its base and
     * layout are inherited. Otherwise the default base and layout are used.
     *
     ...
     *
     * If `other` is a tensor ([marray](@ref MArray::marray)), then its base is
     * inherited. Otherwise the default base is used.
     *
     * @param other     The tensor, view, or partially-indexed tensor to copy.
     *
     ...
     *
     * If `other` is a tensor ([marray](@ref MArray::marray)), then its layout
     * is inherited. Otherwise the default layout is used.
     *
     * @param other     The tensor, view, or partially-indexed tensor to copy.
     *
     ...
     * @param other     The tensor, view, or partially-indexed tensor to copy.
     *
     * @param fortran   The token [FORTRAN](@ref MArray::FORTRAN) or
     ...
     * Move constructor.
     *
     * @param other The tensor to move from. It is left in the same state as if
     * @ref reset() was called.
     */
    marray(marray&& other) { reset(std::move(other)); }
     ...
     *              lengths, including initializer lists.
     *
     * @param layout    The layout to use, either [ROW_MAJOR](@ref
     *                  MArray::ROW_MAJOR) or [COLUMN_MAJOR](@ref
     *                  MArray::COLUMN_MAJOR).
     */
     ...
     *              lengths, including initializer lists.
     *
     * @param base    The base to use, either [BASE_ZERO](@ref
    ...
    /**
     * Create a tensor of the specified shape and with the given base and
     * layout, and without initializing tensor elements.
     *
     * @param len   The length of each dimension. May be any one-dimensional
     *              container type with elements convertible to tensor
     *              lengths, including initializer lists.
     ...
     *                expression ([bcast](@ref MArray::slice::bcast)) is not
     *                valid. The default layout and base are used.
     */
#if !MARRAY_DOXYGEN
     ...
     *
     * @param other The tensor from which to move. It is left in the state as if
     * @ref reset() were called on it.
     *
     * @return      A reference to the marray.
     */
#if MARRAY_DOXYGEN
...
#else
    // The following is necessary to work around a defect in C++ whereby
    // explicit constructors are NOT disregarded in overload resolution, leading
    // to an ambiguity between this constructor and reset(array_1d) in something
    // like x.reset({1,2})
    template <typename T,
              typename = std::enable_if_t<std::is_same_v<T, marray>>>
    ...
    /**
     * Re-initialize the tensor by copying another tensor or tensor view, with
     * specified layout.
     *
     * If `other` is a tensor ([marray](@ref MArray::marray)), then its base is
     * inherited. Otherwise the default base is used.
    ...
    /**
     * Re-initialize the tensor by copying another tensor or tensor view, with
     * FORTRAN/MATLAB layout.
     *
     * @param other   The tensor, view, or partially-indexed tensor to copy.
     *
     * @param fortran The token [FORTRAN](@ref MArray::FORTRAN) or [MATLAB](@ref
     *                MArray::MATLAB).
     *
     * @return        A reference to the marray.
     */
     ...
     * Re-initialize to a tensor of the specified shape and fill value.
     *
     * @param len           The length of each dimension. May be any
     *                      one-dimensional container type with elements
     *                      convertible to tensor lengths, including initializer
     *                      lists.
     *
     * @param uninitialized The token @ref uninitialized.
     *
     ...
     *             lengths, including initializer lists.
     *
     * @param base The base to use, either [BASE_ZERO](@ref MArray::BASE_ZERO)
     *             or [BASE_ONE](@ref MArray::BASE_ONE) (a.k.a. [FORTRAN](@ref
     *             MArray::FORTRAN) or [MATLAB](@ref MArray::MATLAB)).
     *
     * @return     A reference to the marray.
     */
    ...
    /**
     * Re-initialize to a tensor of the specified shape and with the given base
     * and without initializing tensor elements.
     *
     * @param len   The length of each dimension. May be any one-dimensional
     *              container type with elements convertible to tensor
     *              lengths, including initializer lists.
    ...
    /**
     * Re-initialize to a tensor of the specified shape and with the given base
     * and layout, and without initializing tensor elements.
     *
     * @param len           The length of each dimension. May be any
     *                      one-dimensional container type with elements
     *                      convertible to tensor lengths, including initializer
     ...
     *
     * @param base          The base to use, either [BASE_ZERO](@ref
     *                      MArray::BASE_ZERO) or [BASE_ONE](@ref
     *                      MArray::BASE_ONE) (a.k.a. [FORTRAN](@ref
     *                      MArray::FORTRAN) or [MATLAB](@ref MArray::MATLAB)).
     *
     * @param layout        The layout to use, either [ROW_MAJOR](@ref
     *                      MArray::ROW_MAJOR) or [COLUMN_MAJOR](@ref
     ...
     * layout, with the given fill value.
     *
     * @param len     The length of each dimension. May be any one-dimensional
     *                container type with elements convertible to tensor
     *                lengths, including initializer lists.
...
#if MARRAY_DOXYGEN
    marray&
    reset(const array_1d<len_type>& len, const Type& val, fortran_t fortran)
#else
    ...
    /**
     * Re-initialize to a tensor of the specified shape in FORTRAN/MATLAB
     * layout, without initializing tensor elements.
     *
     * @param len           The length of each dimension. May be any
     *                      one-dimensional container type with elements
     *                      convertible to tensor lengths, including initializer
     ...
     * @return       A reference to the marray.
     *
     * @note         Only available when `NDim != ` [DYNAMIC](@ref
     * MArray::DYNAMIC) or 1.
     */
#if !MARRAY_DOXYGEN
    template <int NDim_ = NDim>
     ...
     * the new shape are retained. Any new elements are initialized as
     * requested.
     *
     ...
     */
    stride_type size() const { return storage_.size; }
