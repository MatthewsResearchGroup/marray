...
: type(layout == DEFAULT_LAYOUT ? MARRAY_DEFAULT_DPD_LAYOUT.type
       : layout == ROW_MAJOR
           ? MARRAY_PASTE(MARRAY_DEFAULT_DPD_LAYOUT, _ROW_MAJOR).type
...
template <typename Type, typename Derived, bool Owner> class dpd_marray_base;

template <typename Type> class dpd_marray_view;
...
class irrep_iterator
{
  ...
  public:
    ...
    : irrep_(irrep),
      ndim_(ndim),
      irrep_bits_(nirrep == 1   ? 0
                  : nirrep == 2 ? 1
                  : nirrep == 4 ? 2
                                : 3),
      irrep_mask_(nirrep - 1),
      nblock_(ndim == 0 ? (irrep == 0 ? 1 : 0)
                        : 1u << (irrep_bits_ * (ndim - 1)))
    {
        MARRAY_ASSERT(ndim >= 0);
        MARRAY_ASSERT(nirrep == 1 || nirrep == 2 || nirrep == 4 || nirrep == 8);
    ...
    void reset() { block_ = std::numeric_limits<unsigned int>::max(); }
...
struct dpd_base
{
    ...
    static dim_vector default_depth(dpd_layout layout, int ndim)
    {
        ...
        auto is_power_of_two = [&](unsigned x) { return (x & (x - 1)) == 0; };

        auto log2_ceil = [&](unsigned x)
        { return log2_floor(x) + !is_power_of_two(x); };

        if (layout == BALANCED_ROW_MAJOR || layout == BALANCED_COLUMN_MAJOR)
        {
        ...
        else if (layout == PREFIX_ROW_MAJOR || layout == PREFIX_COLUMN_MAJOR)
        {
    ...
    void set_tree(dim_vector depth)
    {
        ...
        for (auto i : depth) MARRAY_ASSERT(i < ndim);
        ...
        for (auto d : reversed_range(ndim))
        {
            ...
            while (i < (int)depth.size())
            {
                ...
                {
                    MARRAY_ASSERT(
                        i + 1 < (int)depth.size() && depth[i + 1] == d + 1);
        ...
        for (auto i : range(ndim - 1))
        {
            ...
            for (auto irr1 : range(nirrep_))
            {
                ...
                for (auto irr2 : range(nirrep_))
                    size_[next][irr1] +=
                        size_[2 * i][irr1 ^ irr2] * size_[2 * i + 1][irr2];
            }
        }
    }
    ...
    void get_block(const U& irreps, V& len, W& data, X& stride) const
    {
        ...
        for (auto i : reversed_range(ndim2 / 2))
        {
            ...
            dpd_stride[2 * i + 1] =
                dpd_stride[2 * i] * size_[2 * i][dpd_irrep[2 * i]];

            stride_type offset = off_[2 * i][dpd_irrep[2 * i]]
                               * dpd_stride[2 * i]
                               + off_[2 * i + 1][dpd_irrep[2 * i + 1]]
                               * dpd_stride[2 * i + 1];
            for (auto irr2 : range(dpd_irrep[2 * i + 1]))
            {
                auto irr1 = irr2 ^ dpd_irrep[parent_[2 * i]];
                offset += size_[2 * i][irr1]
                        * size_[2 * i + 1][irr2]
                        * dpd_stride[2 * i];
            }
        ...
        {
            stride[i] =
                dpd_stride[leaf_[perm_[i]]] * stride_[perm_[i]][irreps[i]];
    ...
    void slice(int idx, const dpd_index& x)
    {
        ...
        irrep_ ^= x.irrep();
        off_[leaf_[perm_[idx]]][x.irrep()] +=
            x.idx() * stride_[perm_[idx]][x.irrep()];
        fixed_[leaf_[perm_[idx]]] = x.irrep();
    }

    template <typename Slice1, typename Slice2, typename... Slices>
    void
    slice(int idx, const Slice1& s1, const Slice2& s2, const Slices&... slices)
    {
        slice(idx, s1);
    ...
    slice(const Slices&... slices)
    {
        ...
        std::array<bool, sizeof...(Slices)> erase{
            std::is_same<Slices, dpd_index>::value...};
        dim_vector to_erase;
    ...
    {
        for (auto i : range(slices.size())) slice(i, slices[i]);
    }
};
...
class dpd_marray_base : protected detail::dpd_base
{
  ...
  protected:
               ...
               dpd_layout layout = DEFAULT_LAYOUT)
    {
        reset(irrep,
              ...
              len,
              ptr,
              default_depth(layout, len.length(0)),
              layout.base());
    }
    ...
    template <typename View, typename Func> void for_each_block_(Func&& f) const
    {
        ...
        {
            irreps[0] = irrep();
            for (auto i : range(1, ndim)) irreps[0] ^= irreps[i] = it.irrep(i);
    ...
    template <typename Tp, typename Func> void for_each_element_(Func&& f) const
    {
        typedef Tp* Ptr;
  ...
  public:
    ...
    static stride_type size(int irrep, const array_2d<len_type>& len)
    {
        ...
        for (auto irr : range(nirrep)) size[irr] = len_[0][irr];
    ...
    {
        for_each_block_<marray_view<ctype, NDim>>(
            std::forward<Func>(f),
            std::make_integer_sequence<int, NDim>{});
    }

    template <int NDim, typename Func> void for_each_block(Func&& f)
    {
        for_each_block_<marray_view<Type, NDim>>(
            std::forward<Func>(f),
            std::make_integer_sequence<int, NDim>{});
    }
    ...
    template <int NDim, typename Func> void for_each_element(Func&& f)
    {
        for_each_element_<Type>(std::forward<Func>(f),
                                std::make_integer_sequence<int, NDim>{});
    }
    ...
    {
        return row_view<const len_type>{{num_irreps()},
                                        &len_[permutation(dim)][0]};
    }
    ...
    matrix<len_type> lengths() const
    {
        ...
        for (auto i : range(dimension()))
            for (auto j : range(num_irreps())) len[i][j] = length(i, j);
        return len;
    }

    auto irrep() const { return irrep_; }

    auto num_irreps() const { return nirrep_; }

    auto& permutation() const { return perm_; }
    ...
    int dimension() const { return perm_.size(); }
};
