...
class marray_view
: public marray_base<Type, NDim, marray_view<Type, NDim>, false>
{
  ...
  public:
...
#else
    marray_view(const marray_view& other) { reset(other); }
    ...
    marray_view(const std::vector<T>& v,
                std::enable_if_t<(NDim == 1 || NDim == DYNAMIC)
                                 && std::is_convertible_v<const T*, pointer>>* =
                    nullptr)
#else
    marray_view(const std::vector<T>& v)
#endif
    ...
    marray_view(
        std::vector<T>& v,
        std::enable_if_t<(NDim == 1 || NDim == DYNAMIC)
                         && std::is_convertible_v<T*, pointer>>* = nullptr)
#else
     ...
     * and the default base and layout.
     *
     ...
     *              const-qualified.
     */
    marray_view(const array_1d<len_type>& len, pointer ptr) { reset(len, ptr); }

    /**
     * Construct a view that wraps a raw data pointer, using the provided shape
     * and base, and the default layout.
     *
     * @param len   The lengths of the tensor dimensions. May be any one-
     *              dimensional container whose elements are convertible to
     *              tensor lengths, including initializer lists.
     ...
     *              const-qualified.
     *
     * @param base  One of [BASE_ZERO](@ref MArray::BASE_ZERO), [BASE_ONE](@ref
     * MArray::BASE_ONE), [FORTRAN](@ref MArray::FORTRAN), or [MATLAB](@ref
     * MArray::MATLAB).
     */
    ...
    /**
     * Construct a view that wraps a raw data pointer, using the provided shape
     * and layout, and the default base.
     *
     * @param len   The lengths of the tensor dimensions. May be any one-
     *              dimensional container whose elements are convertible to
     *              tensor lengths, including initializer lists.
     ...
     *              const-qualified.
     *
     * @param stride    The strides along each dimension, or a layout (one of
     *                  [ROW_MAJOR](@ref MArray::ROW_MAJOR), [COLUMN_MAJOR](@ref
     *                  MArray::COLUMN_MAJOR), [FORTRAN](@ref MArray::FORTRAN),
     *                  [MATLAB](@ref MArray::MATLAB)). The stride is the
     *                  distance in memory (in units of the value type) between
     *                  successive elements along this direction. In general,
     *                  the strides need not be defined such that elements have
     *                  unique locations, although such a view should not be
     *                  written into. Strides may also be negative. In this
     *                  case, `ptr` still refers to the location of the "first"
     *                  element (`index == base`), although this is not the
     *                  lowest address of any tensor element.
     */
    marray_view(const array_1d<len_type>& len,
                pointer ptr,
     ...
     *              const-qualified.
     *
     * @param base  The base for each index, or a standard base (one of
     *              [BASE_ZERO](@ref MArray::BASE_ZERO), [BASE_ONE](@ref
     *              MArray::BASE_ONE), [FORTRAN](@ref MArray::FORTRAN),
     *              [MATLAB](@ref MArray::MATLAB)). The base is the minimum
     *              value of the index. For example, a tensor with lengths
     *              (2,3,4) and base (1,6,0) can have valid indices along the
     *              three dimensions in the ranges [1,3), [6,9), and [0,4),
     *              respectively.
     *
     * @param stride    The strides along each dimension, or a layout (one of
     *                  [ROW_MAJOR](@ref MArray::ROW_MAJOR), [COLUMN_MAJOR](@ref
    ...
    /**
     * Construct a view that wraps a raw data pointer, using the provided shape
     * and standard FORTRAN/MATLAB indexing.
     *
     * @param len   The lengths of the tensor dimensions. May be any one-
     *              dimensional container whose elements are convertible to
     *              tensor lengths, including initializer lists.
     ...
     *              const-qualified.
     *
     * @param fortran   Either the token [FORTRAN](@ref MArray::FORTRAN) or
    ...
    /**
     * Construct a view that wraps a raw data pointer, using the provided
     * extents and the default layout.
     *
     * @note This constructor provides functionality similar to the
     *       multidimensional array declarations in FORTRAN, e.g. `real,
     *       dimension(begin1:end1, begin2:end2, ...) :: array`, except that the
     *       upper bounds (`end`) are one greater than in the corresponding
     *       FORTRAN statement. This is by design such that the basic
     *       constructors with only `len` are equivalent to `begin = [0,...]`
     *       and `end = len`.
     *
     * @param begin   The smallest values of each index. May be any one-
     *                dimensional container whose elements are convertible to
     *                tensor lengths, including initializer lists. These values
     *                are the same as the tensor @ref base().
     *
     * @param end   One plus the largest values of each index. May be any one-
     *              dimensional container whose elements are convertible to
     *              tensor lengths, including initializer lists. The length of
     *              index `i` is equal to `end[i]-begin[i]`.
     *
     * @param ptr   A pointer to the tensor element with all zero inidices.
     *              If this is a mutable view, then the pointer may not be
     *              const-qualified.
     ...
     *                  written into. Strides may also be negative. In this
     *                  case, `ptr` still refers to the location of the "first"
     *                  element (`index == begin`), although this is not the
     *                  lowest address of any tensor element.
     */
    marray_view(const array_1d<len_type>& begin,
                const array_1d<len_type>& end,
     ...
     *
     * The `k`th index `i` in the shifted view is equivalent to an index
     * `i+n[k]` in the original view.
     *
    ...
    void shift(const array_1d<len_type>& n)
    {
        ...
        for (auto dim : range(dimension())) shift(dim, n_[dim]);
    }
     ...
     * @param dim  The dimension along which to shift the view.
     */
    void shift_down(int dim) { shift(dim, length(dim)); }
     ...
     *
     * @param perm  The permutation vector. May be any one-dimensional container
     *              type whose elements are convertible to `int`, including
     *              initializer lists. The values must form a permutation of
     *              `[0,NDim)`, where `NDim` is the number of tensor dimensions.
     */
     ...
     * indexing into dimension `perm[i]` of the original view.
     *
     * @param perm  The permutation vector. May be any set of integral types
     *              convertible to `int`. The values must form a permutation of
     *              `[0,NDim)`, where `NDim` is the number of tensor dimensions.
     */
#if MARRAY_DOXYGEN
     ...
     * consistent stride (i.e. those dimensions must be contiguous). The base of
     * an index which is formed by combining multiple indices in the original
     * view is equal to the base of index with smallest stride.
     *
     * This overload is only available for dynamic views (`NDim == `
     * [DYNAMIC](@ref MArray::DYNAMIC)).
     *
     * @param split The "split" or "pivot" vector. The number of split
     *              points/pivots must be equal to the number of dimensions in
     *              the lowered view minus one. Dimensions `[0,split[0])`
     *              correspond to the first dimension of the return view,
     *              dimensions `[split[K-1],N)` correspond to the last dimension
     *              of the returned view, and dimensions `[split[i-1],split[i])`
     *              correspond to the `i`th dimension of the return view
     *              otherwise, where `N` is the dimensionality of the original
     *              tensor and `K` is the number of dimensions in the
     *              lower-dimensional view. The split points must be in
     *              increasing order. May be any one-dimensional container type
     *              whose elements are convertible to `int`, including
     *              initializer lists.
     */
#if MARRAY_DOXYGEN
     ...
     *
     * In order to be successfully reshaped, consecutive groups of indices which
     * have contiguous storage in the original view (i.e. the indices in the
     * group are collectively row-major or column-major with no gaps in storage)
     * must map exactly (meaning, must have the same total size) onto
     * corresponding groups of indices in the reshaped view. It is not necessary
     * for the entire view to have contiguous storage so long as groups of
     * indices can be mapped independently. For each group of indices, the
     * layout in the new view is determined by the base (lowest) stride for the
     * group combined with row-major or column-major layout as determined by the
     * layout of the original group. If the original group is a single index,
     * layout is inferred from relative strides of surrounding indices. In
     * particular this means that mapping index j in ...ijk... as a singleton
     * group is only legal is stride[i] > stride[j] > stride[k] (row-major) or
     * stride[i] < stride[j] < stride[k] (column-major).
     *
     * @param lengths The lengths of the reshaped view. If `NewNDim` is not
     *                [DYNAMIC](@ref MArray::DYNAMIC), then the size of
     *                `lengths` must equal `NewNDim`.
     */
#if MARRAY_DOXYGEN
     ...
     *
     * For the indicated dimension, an index of `i` in the reversed view
     * corresponds to an index of `n-1-i` in the original view, where `n` is the
     * tensor length along that dimension.
     *
     ...
     */
    void swap(marray_view& other) { base_class::swap(other); }
